...
```bash
sudo -u postgres createuser --pwprompt hms
sudo -u postgres createdb --owner=hms hms_control
```

Give `hms` the `CREATEDB` privilege — `onboard.mjs` creates a database per
hospital, and `reset-tenant.mjs` drops them:

```bash
sudo -u postgres psql -c "ALTER ROLE hms CREATEDB;"
```

## 3. The code

```bash
sudo mkdir -p /srv/hms && sudo chown $USER /srv/hms
...
CONTROL_PLANE_URL="postgresql://hms:PASSWORD@localhost:5432/hms_control?schema=public"
DATABASE_URL="postgresql://hms:PASSWORD@localhost:5432/hms_control?schema=public"
...
BACKUP_KEEP_DAYS=30
BACKUP_UPLOAD_CMD="rclone copy --config /home/hms/.rclone.conf"
```
